...
class Car(object):
    def __init__(self, carID, length, pos, orient):
        ...
        self._steering_limit = math.pi / 6.0
        self._length = length
        self._length_r = self._length / 2.0
        ...
        self._A = np.array([[-8.0, -4.199], [8.0, 0.0]])
        self._B = np.array([[2], [0]])
        self._C = np.array([[0, 12.6]])

        # Model configuration
        p.resetBasePositionAndOrientation(
            self.id, [0.0, 0.0, pos[2]], p.getQuaternionFromEuler([0.0, 0.0, 0.0])
        )
    ...
    def step(self, cmd, contact, dt):
        ...
        w = throttle * math.tan(self._steering_angle) * math.cos(beta) / self._length
