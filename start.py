...
class Host:
    ...
    def run(self, hosts, mutex):
        ...
        if not self.is_local:
            argv = ["ssh", "-t", "%s@%s.local" % (self.username, self.hostname), *argv]
        ...
        prefix = "{}{:<{offset}}  | {}".format(
            ...
            self.hostname,
            Fore.RESET,
            offset=max([len(x.hostname) for x in hosts]),
        )
        buffer = b""
        while self.running:
            try:
        ...
        self.print("{}Shut down {}{}".format(self.color, self.hostname, Fore.RESET))
    ...
    def stop(self):
        if self.running:
            self.print(
                "{}Shutting down {}...{}".format(self.color, self.hostname, Fore.RESET)
            )
...
if __name__ == "__main__":
    ...
    threads += [
        Thread(target=x.run, args=(hosts, mutex)) for x in hosts if not x.roscore
    ]
