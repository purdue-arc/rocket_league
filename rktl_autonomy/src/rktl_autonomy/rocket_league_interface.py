...
class RocketLeagueInterface(ROSInterface):
    ...
    def __init__(self, eval=False, launch_file=['rktl_autonomy', 'rocket_league_train.launch'], launch_args=[], run_id=None, env_counter=None):
        super().__init__(node_name='rocket_league_agent', eval=eval, launch_file=launch_file, launch_args=launch_args, run_id=run_id)
        # Constants
        self.instance_id = env_counter.get_current_counter()
        # Actions
        env_counter.count_env()
