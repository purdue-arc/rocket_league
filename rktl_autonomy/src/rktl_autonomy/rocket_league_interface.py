...
class RocketLeagueInterface(ROSInterface):
    ...
    def action_space(self):
        ...
        return Box(
            # throttle, steering
            low=np.array(
                [self._MIN_THROTTLE_EFFORT, self._MIN_STEERING_EFFORT], dtype=np.float32
            ),
            high=np.array(
                [self._MAX_THROTTLE_EFFORT, self._MAX_STEERING_EFFORT], dtype=np.float32
            ),
        )
    ...
    def observation_space(self):
        ...
        return Box(
            ...
            low=np.array(
                [
                    -self._FIELD_HEIGHT / 2,
                    -self._FIELD_WIDTH / 2,
                    -pi,
                    -self._MAX_OBS_VEL,
                    -self._MAX_OBS_ANG_VEL,
                    -self._FIELD_HEIGHT / 2,
                    -self._FIELD_WIDTH / 2,
                    -self._MAX_OBS_VEL,
                    -self._MAX_OBS_VEL,
    ...
    def _has_state(self):
        ...
        return (
            self._car_odom is not None
            and self._ball_odom is not None
    ...
    def _get_state(self):
        ...
        if not self.observation_space.contains(observation):
            rospy.logerr(
                "observation outside of valid bounds:\nObservation: %s", observation
            )
        ...
        goal_dist_sq = np.sum(
            np.square(ball[0:2] - np.array([self._FIELD_HEIGHT / 2, 0]))
        )
        ...
        if (
            abs(x) > self._FIELD_HEIGHT / 2 - self._WALL_THRESHOLD
            or abs(y) > self._FIELD_WIDTH / 2 - self._WALL_THRESHOLD
        ):
