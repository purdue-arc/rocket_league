...
class Simulator(object):
    ...
    def effort_cb(self, effort_msg, car_id):
        ...
        self.sim.set_car_command(car_id, (effort_msg.throttle, effort_msg.steering))
        self.cmd_lock.release()

    def cmd_cb(self, cmd_msg, car_id):
        """Sets a car's command with the velocity and curvature. (overwrites effort)."""
        self.cmd_lock.acquire()
        self.sim.set_car_command(car_id, (cmd_msg.velocity, cmd_msg.curvature))
        self.cmd_lock.release()
    ...
    #     if not rclpy_param:
    #         if returnValue:
    #             rclpy.logfatal(f'invalid file path: {path}')
    #         return None
    #     else:
